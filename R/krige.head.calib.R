...
#' @param \code{trendMaxDistFrac} defines the calibration type and range for the threshold parameter for the distance between an observation and estimation point (as a fraction of \code{maxdist}) at which minimal weight should be placed on the observation (see Rivoirard et al. 2011).
...
#' varigram.model <- vgm(psill=25, model="Mat", range=5000, nugget=5, kappa = 0.5)
#'
#' # Calibrate the mapping parameters with 25% of the data randomly selected.
#' calib.results <- krige.head.calib(formula=f, grid=DEM, data=obs.data, newdata=0.25,
#' data.errvar.colname='total_err_var', model = varigram.model,  fit.variogram.type=1,
#' debug.level=1)
#'
...
#'                trendMaxDistFrac=NA,nug = NA, psill_model_1 = NA, range_model_1 = NA,
...
#' model=varigram.model, smooth.std=params.all[['smooth.std']], maxdist=params.all[['maxdist']],
...
krige.head.calib <-
  function(formula = as.formula("head ~ elev + MrVBF + MrRTF + smoothing"),
    ...
    if (class(use.cluster)[1] == "SOCKcluster" |
      ...
      use.local.cluster <- makeCluster(clus, type = "SOCK")
    } else {
      use.local.cluster = F;
    }
    ...
    if (nclus>1) {
      clusterEvalQ(use.local.cluster, library(gstat))
      clusterEvalQ(use.local.cluster, library(raster))
    }
    ...
    if (!is.null(maxdist)) {
      ...
      if (length(maxdist) >1) {
        ...
        if (!is.logical(use.local.cluster)) {
          ...
          grid.dist <- do.call("rbind", parLapply(use.local.cluster, grid.tmp, function(lst) predict(g, lst)))
        } else {
    ...
    if (!is.null(trendMaxDistFrac)) {
      if (length(trendMaxDistFrac)==1) {
        # Add fixed parameter value to the lookup table This is requied so that only a subset of parameters can be calibrated.
        param.IntegerLookupTable[['trendMaxDistFrac']] = cbind(1, trendMaxDistFrac)
      } else if (length(trendMaxDistFrac) == 2) {
        nParams = nParams + 1

        param.Names[nParams] = 'trendMaxDistFrac'
        param.bounds = rbind(param.bounds, c(trendMaxDistFrac[1], trendMaxDistFrac[2]))
        doIntegerCalib[nParams] = FALSE
      } else {
        nParams = nParams + 1

        param.Names[nParams] = 'trendMaxDistFrac'
        param.bounds = rbind(param.bounds, c(1, length(trendMaxDistFrac)))
        doIntegerCalib[nParams] = TRUE

        param.IntegerLookupTable[[param.Names[nParams]]] = cbind(seq(1, length(trendMaxDistFrac), 1), trendMaxDistFrac)
      }
    }
