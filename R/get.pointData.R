get.pointData <- function(formula = as.formula("head ~ elev + MrVBF + MrRTF"), data, grid, mrvbf.pslope = 1.0, mrvbf.ppctl = 1.0, smooth.std = 1.0, smooth.ncells=11, smoothingKernal = NULL, use.cluster=TRUE, debug.level=0 ) {
    ...
    if (use.DEMsmoothing) {
      ...
      # Calc. smoothed DEM
      data = get.smoothedDEM(data, grid, smooth.std = smooth.std,  smoothingKernal=smoothingKernal, smooth.ncells=smooth.ncells, use.cluster=use.cluster ,debug.level=debug.level)
