get.allData <- function(formula = as.formula("head ~ elev + MrVBF + MrRTF"), data, grid, mrvbf.pslope = 1.0, mrvbf.ppctl = 1.0, smooth.std = 1.0, smoothingKernal = NULL,  smooth.ncells=11, min.sepDist = 5, maxStoredGrids=10,debug.level=0) {
	...
	if (use.DEMsmoothing) {
	  ...
	  # Calc. smoothed DEM
	  if (is.null(data)) {
	    newdata = get.smoothedDEM(NULL, newdata, smooth.std = smooth.std, smoothingKernal=smoothingKernal, smooth.ncells=smooth.ncells, maxStoredGrids, debug.level=debug.level)
	  } else {
	    newdata = get.smoothedDEM(newdata, grid, smooth.std = smooth.std, smoothingKernal=smoothingKernal, smooth.ncells=smooth.ncells, maxStoredGrids, debug.level=debug.level)	  
	  }
