get.smoothedDEM <- function(data, grid, grid_buffer=NULL, smooth.std = 1.0, smooth.ncells=smooth.ncells, smoothingKernal=NULL, maxStoredGrids=10, debug.level=0 ) {
  ...
  if (!is.null(pkg.env$smoothDEM.grid)) {
    ...
    grid.smoothDEM.params = sp::gridparameters(pkg.env$smoothDEM.grid)
  ...
  if (do.smoothedDEM) {
      ...
      sp::fullgrid(smoothDEM.grid) = TRUE;
  ...
  if (!is.null(data)) {
    ...
    grd.asRaster = raster::raster(grid,layer='smoothDEM');
    data$smoothDEM = NULL;
    tmp = raster::extract(grd.asRaster, data, method='bilinear');
