get.MrVBF <- function(data = NULL, grid, pslope=NULL, ppctl=NULL, return.MrVBF=TRUE, return.MrRTF=TRUE, maxStoredGrids=10, debug.level=0) {
  ...
  if (do.MrVBF || do.MrRTF) {
    ...
    if (!file.exists(paste(fname.MrVBF,'.asc',sep='')) || !file.exists(paste(fname.MrRTF,'.asc',sep=''))) {
      ...
      if (pkg.env$saga.has.MrVFB) {
        ...
        if (!file.exists(fname.dem.sgrd) || !file.exists(fname.dem.sdat)) {
          ...
          rsaga.import.gdal(fname.dem,env = pkg.env$saga.settings);
        }
      }
      ...
      if (debug.level>0)
    	...
    	if (do.MrVBF) {
    	  rsaga.sgrd.to.esri(paste(fname.MrVBF,'.sgrd',sep=''),env = pkg.env$saga.settings);
    	  file.remove(paste(fname.MrVBF,'.sgrd',sep=''));
    	  file.remove(paste(fname.MrVBF,'.mgrd',sep=''));
    	  file.remove(paste(fname.MrVBF,'.sdat',sep=''));
    	}
    	if (do.MrRTF) {
    	  rsaga.sgrd.to.esri(paste(fname.MrRTF,'.sgrd',sep=''),env = pkg.env$saga.settings);
    ...
    if (do.MrVBF) {
	    ...
	    if (!is.na(ind) && sum(ind)>0) {
    	  ...
    	  gridded(grid.MrVBF) = TRUE;
    	  fullgrid(grid.MrVBF) = TRUE;
    	...
    	# Get grid geometry
    	grid.MrVBF.params = gridparameters(grid.MrVBF)
    }
    if (do.MrRTF) {
    	var.name.all = names(pkg.env$MrRTF.grid)
    	...
    	if (!is.na(ind) && sum(ind)>0) {
    	  ...
    	  gridded(grid.MrRTF) = TRUE;
    	  fullgrid(grid.MrRTF) = TRUE;
    	...
    	# Get grid geometry
    	grid.MrRTF.params = gridparameters(grid.MrRTF)
    }
  ...
  if (!is.null(data)) {
      ...
      MrVBF.asRaster = raster(grid.MrVBF,layer=var.name.MrVBF);
      data$MrVBF = NULL;
      if (debug.level>0)
        message('... Interpolating grid to point locations.');

      tmp = extract(MrVBF.asRaster, data, method='bilinear');
      data$tmpName =  tmp;
      ...
      # DEM values at fixed head points beyond the mappng area (eg coastal points
      # with a fixed head of zero just beyond the DEM extent)
      MrRTF.asRaster = raster(grid.MrRTF,layer=var.name.MrRTF);
      data$MrRTF = NULL;
      if (debug.level>0)
        message('... Interpolating grid to point locations.');

      tmp = extract(MrRTF.asRaster, data, method='bilinear');
      data$tmpName =  tmp;
      ncols = length(names(data))
