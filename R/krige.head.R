...
krige.head <- function(
  ...
  if (!do.grid.est) {
    ...
    tmp = raster::extract(dem.asRaster, newdata, method='bilinear');
    filt = is.na(tmp);
    if (any(filt)) {
  ...
  if (!is.null(data.fixedHead)) {
    ...
    tmp = raster::extract(dem.asRaster, data.fixedHead, method='bilinear');
    filt = is.na(tmp);
    if (any(filt)) {
...
#        if (!use.extraTerms || (use.extraTerms && all(is.na(match(use.extraTerms.names,colnames[i])))))
  ...
  if (do.calibration) {
    ...
    if (fit.variogram.type==1){
      ...
      model = gstat::vgm(nugget=nug, psill=psill, range=range, kappa=kappa, anis=c(ang1,anis1),model=modelType)
    }
  }
  ...
  if (!is.null(grid.landtype.colname) || !is.null(data.fixedHead)) {
    ...
    g = gstat::gstat(NULL, 'trend', formula=formula, data=data, model=model, nmin=nmin, nmax=nmax, maxdist = maxdist, omax=omax, force=TRUE, weights=data.weights)
  }
  ...
  if (!is.null(grid.landtype.colname)) {
    ...
    tmp = raster::extract(raster(grid,grid.landtype.colname), data, method='simple');
    if (any(is.na(tmp))) {
    ...
    if (!is.null(newdata)) {
      tmp = raster::extract(raster(grid,grid.landtype.colname), newdata, method='simple');
      newdata$tmpName =  tmp;
  ...
  if (!is.null(data.fixedHead)) {
    ...
    data.fixedHead.easting = sp::coordinates(data.fixedHead)[,1]
    data.fixedHead.northing = sp::coordinates(data.fixedHead)[,2]
    data.easting = sp::coordinates(data)[,1]
    data.northing = sp::coordinates(data)[,2]
  ...
  if (do.grid.est) {
    ...
    ncells = dim(sp::coordinates(grid))[1];
    ...
    if (class(use.cluster)[1]=="SOCKcluster" | class(use.cluster)[1]=="PVMcluster" | class(use.cluster)[1]=="spawnedMPIcluster" | class(use.cluster)[1]=="MPIcluster") {
      ...
      if (is.logical(use.cluster)) {
        nclus=1
        if (use.cluster) {nclus = parallel::detectCores(all.tests = TRUE, logical = FALSE)};
    ...
    gs = gstat::gstat(NULL, 'heads', formula=formula, data=data, model=model, nmin=nmin, nmax=nmax, maxdist = maxdist, omax=omax, force=TRUE, weights=data.weights)
    if (use.LandCatagory) {
      gs = gstat::gstat(gs,"land.type", formula = as.formula(paste(grid.landtype.colname,' ~ 1')), grid, nmax=1, nmin=1, model = model.landtype,merge=c("heads","land.type"))
      gs = gstat::gstat(gs, c("heads","land.type"), model = model.landtype.head)
    }
    if (use.FixedHeads) {
      ...
      gs = gstat::gstat(gs,"fixed.head", formula = formula, data=data.fixedHead, model = model.fixedHead, nmin=nmin.fixedHead, nmax=nmax.fixedHead, maxdist = maxdist, omax=omax.fixedHead, force=T,merge=c("heads","fixed.head"))
      gs = gstat::gstat(gs, c("heads","fixed.head"), model = model.fixedHead.head, merge=list(mergeVarIDs))
    }
    ...
    if (nclus>1) {
      ...
      if (nsim==0) {
        ...
        head <- do.call("rbind", parallel::parLapply(cl, newgrid, function(lst) predict(gs, lst, debug.level=debug.level)))
        ...
        head <- do.call("cbind", parallel::parLapply(cl, sims_perCore, function(lst) predict(gs, newdata=grid, nsim=lst, debug.level=debug.level)))
    ...
    if (!is.na(smooth.std) && !is.null(smooth.std) && !use.DEMsmoothing) {
      ...
      # Do smoothing
      if (nsim==0) {
        head.asRaster = raster::focal(raster(head,1), sigmaWeights, na.rm=TRUE);
        var.asRaster = raster::focal(raster(head,2), sigmaWeights, na.rm=TRUE);
        ...
        for (i in 1:nsim) {
          head.asRaster = raster::focal(raster(head,i), sigmaWeights, na.rm=TRUE);
  ...
  if (!use.DEMsmoothing && !is.na(smooth.std)  && smooth.std>0) {
    ...
    if (use.MrVBF)
      grid.MrVBF= raster::raster(grid,layer='MrVBF')
    if(use.MrRTF)
      grid.MrRTF= raster::raster(grid,layer='MrRTF')
    if (use.LandCatagory)
      grid.LandType= raster::raster(grid,layer='LandType')
  }
  ...
  if (nclus>1) {
    ...
    est <- do.call("rbind", parallel::parLapplyLB(cl, splitData, function(lst) 
                                krige.head.crossval(
                                        newdata=lst,
                                        formula=formula,
                                        model=model,
  ...
  sp::coordinates(est) = ~Easting + Northing;
  if (debug.level>0)
